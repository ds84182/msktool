...
Future main(List<String> args) async {
  ...
  await Future.wait<dynamic>([
    ...
    readContextFromGameRoot(lfs.currentDirectory, suffix: "").then((context) {
      attr = context;
  ...
  if (islands.length == 1 && islands.single == "all") {
    islands = new ClassKey.byName(attr, "island")
        .lookupCollections()
        .map((collection) => attr.lookupName(collection.id))
        .toList(growable: false);
  }

  final futures = islands.map((islandName) async {
    void print(Object obj) => stdout.writeln("[$islandName] $obj");
    ...
    if (islandStartWorldRefSpec == null) {
      print("Not a valid island (No start world!)");
      return;
    }
    ...
    final UITexture textureId = readField<UITexture>(mapCollection, "TextureName");
    ...
    final textureEntry = pf.findAllEntries(textureId.id).first;
    ...
    final textureImage = new Image.fromBytes(
        texture.width, texture.height, await texture.decoded);
    ...
    int index = 0;
    attr
        .lookupCollections(fishingBucketClass)
        .forEach((fishingBucketCollection) {
      final parent =
          readField<RefSpec>(fishingBucketCollection, "ParentRefSpec");

      if (parent == islandStartWorldRefSpec) {
        final List<CollectionKey> spawnList = readField(
            fishingBucketCollection, "Tuning_FishingSpawnLocationList");
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      print(
          "   Weights: ${readField(grassRewards, "Tuning${i}_RewardWeights")}");
    ...
    attr.lookupCollections(grassClass).forEach((grassCollection) {
      ...
      if (parent == islandStartWorldRefSpec) {
        ...
        drawCircle(scaledTextureImage, imageX, imageY,
            (1.0 * mapScaleX).toInt(), getColor(0, 255, 0));
      }
    });
    ...
    attr.lookupCollections(treeClass).forEach((collection) {
      ...
      if (parent == islandStartWorldRefSpec) {
        ...
        print(
            "   Fruits: ${readField(treeTypeCollection, "Tuning_FruitTypes")}");
        print(
...
//        imageY + imageYHE, (hashColor & 0xFFFFFF) | 0x3F000000);
    ...
    if (const bool.fromEnvironment("DISPLAY")) {
      ...
      new File("map_$islandName.png").writeAsBytesSync(encodedPng);
    }
