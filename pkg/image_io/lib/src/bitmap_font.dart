...
class BitmapFont {
  ...
  BitmapFont.fromZip(List<int> fileData) {
    archive.Archive arc = new archive.ZipDecoder().decodeBytes(fileData);

    archive.ArchiveFile font_file;
  ...
  void _parseFnt(XML.XmlDocument xml, Map<int, Image> fontPages,
    ...
    for (var c in font.children) {
      ...
      String name = (c as XML.XmlElement).name.toString();
      if (name == 'info') {
        ...
        for (var page in c.children) {
          if (page is XML.XmlElement) {
            ...
            if (arc != null) {
              archive.ArchiveFile imageFile = _findFile(arc, filename);
              if (imageFile == null) {
        ...
        if (name == 'chars') {
          for (var char in c.children) {
            if (char is XML.XmlElement) {
              ...
              BitmapFontCharacter ch = new BitmapFontCharacter(
                  id,
                  width,
                  height,
                  xoffset,
  ...
  static archive.ArchiveFile _findFile(archive.Archive arc, String filename) {
    for (archive.ArchiveFile f in arc.files) {
      if (f.name == filename) {
        return f;
      }
    }
