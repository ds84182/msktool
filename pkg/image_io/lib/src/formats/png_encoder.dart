...
class PngEncoder extends Encoder {
  ...
  List<int> encodeImage(Image image) {
    ...
    List<int> compressed = new ZLibEncoder(level: level ?? ZLibOption.DEFAULT_LEVEL)
  ...
  int _filterNone(Image image, int oi, int row, List<int> out) {
    ...
    for (int x = 0; x < image.width; ++x) {
      int c = image.getPixelUnsafe(x, row);
      out[oi++] = getRed(c);
      out[oi++] = getGreen(c);
      out[oi++] = getBlue(c);
      if (image.format == Image.RGBA) {
        out[oi++] = getAlpha(image.getPixelUnsafe(x, row));
      }
    }
    return oi;
  }
  ...
  int _filterSub(Image image, int oi, int row, List<int> out) {
    ...
    out[oi++] = getRed(image.getPixelUnsafe(0, row));
    out[oi++] = getGreen(image.getPixelUnsafe(0, row));
    out[oi++] = getBlue(image.getPixelUnsafe(0, row));
    if (image.format == Image.RGBA) {
      out[oi++] = getAlpha(image.getPixelUnsafe(0, row));
    }

    for (int x = 1; x < image.width; ++x) {
      int ar = getRed(image.getPixelUnsafe(x - 1, row));
      int ag = getGreen(image.getPixelUnsafe(x - 1, row));
      int ab = getBlue(image.getPixelUnsafe(x - 1, row));

      int r = getRed(image.getPixelUnsafe(x, row));
      int g = getGreen(image.getPixelUnsafe(x, row));
      int b = getBlue(image.getPixelUnsafe(x, row));

      out[oi++] = ((r - ar)) & 0xff;
      out[oi++] = ((g - ag)) & 0xff;
      out[oi++] = ((b - ab)) & 0xff;
      if (image.format == Image.RGBA) {
        int aa = getAlpha(image.getPixelUnsafe(x - 1, row));
        int a = getAlpha(image.getPixelUnsafe(x, row));
        out[oi++] = ((a - aa)) & 0xff;
      }
    }
  ...
  int _filterUp(Image image, int oi, int row, List<int> out) {
    ...
    for (int x = 0; x < image.width; ++x) {
      int br = (row == 0) ? 0 : getRed(image.getPixelUnsafe(x, row - 1));
      int bg = (row == 0) ? 0 : getGreen(image.getPixelUnsafe(x, row - 1));
      int bb = (row == 0) ? 0 : getBlue(image.getPixelUnsafe(x, row - 1));

      int xr = getRed(image.getPixelUnsafe(x, row));
      int xg = getGreen(image.getPixelUnsafe(x, row));
      int xb = getBlue(image.getPixelUnsafe(x, row));

      out[oi++] = (xr - br) & 0xff;
      out[oi++] = (xg - bg) & 0xff;
      out[oi++] = (xb - bb) & 0xff;
      if (image.format == Image.RGBA) {
        int ba = (row == 0) ? 0 : getAlpha(image.getPixelUnsafe(x, row - 1));
        int xa = getAlpha(image.getPixelUnsafe(x, row));
        out[oi++] = (xa - ba) & 0xff;;
      }
    }
  ...
  int _filterAverage(Image image, int oi, int row, List<int> out) {
    ...
    for (int x = 0; x < image.width; ++x) {
      int ar = (x == 0) ? 0 : getRed(image.getPixelUnsafe(x - 1, row));
      int ag = (x == 0) ? 0 : getGreen(image.getPixelUnsafe(x - 1, row));
      int ab = (x == 0) ? 0 : getBlue(image.getPixelUnsafe(x - 1, row));
      ...
      if (image.format == Image.RGBA) {
        int aa = (x == 0) ? 0 : getAlpha(image.getPixelUnsafe(x - 1, row));
        int ba = (row == 0) ? 0 : getAlpha(image.getPixelUnsafe(x, row - 1));
        int xa = getAlpha(image.getPixelUnsafe(x, row));
  ...
  int _filterPaeth(Image image, int oi, int row, List<int> out) {
    ...
    for (int x = 0; x < image.width; ++x) {
      ...
      int cr = (row == 0 || x == 0) ? 0 : getRed(image.getPixelUnsafe(x - 1, row - 1));
      int cg = (row == 0 || x == 0) ? 0 : getGreen(image.getPixelUnsafe(x - 1, row - 1));
      int cb = (row == 0 || x == 0) ? 0 : getBlue(image.getPixelUnsafe(x - 1, row - 1));
      ...
      if (image.format == Image.RGBA) {
        int aa = (x == 0) ? 0 : getAlpha(image.getPixelUnsafe(x - 1, row));
        int ba = (row == 0) ? 0 : getAlpha(image.getPixelUnsafe(x, row - 1));
        int ca = (row == 0 || x == 0) ? 0 : getAlpha(image.getPixelUnsafe(x - 1, row - 1));
        int xa = getAlpha(image.getPixelUnsafe(x, row));
  ...
  int _crc(String type, List<int> bytes) {
    int crc = archive.getCrc32(type.codeUnits);
    return archive.getCrc32(bytes, crc);
  }
